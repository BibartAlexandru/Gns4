...
public class PacketHeader implements ByteSerializable<PacketHeader>{
  ...
  public static final int HEADER_CHECKSUM_LEN = 2 ;
	public static final int NR_BYTES = 2*IPv4NetworkAddress.NR_BYTES + HEADER_CHECKSUM_LEN + 3 * SIZEOF_BYTE + SIZEOF_INT * 5;
	
  public PacketHeader(
    IPv4NetworkAddress sourceIp, IPv4NetworkAddress destIp
  )
  {
    super();
    this.version = 4 ;
	...
	public PacketHeader(
    byte version,
   int identification, byte flags, int fragmentOffset, int ttl, byte protocol, 
  byte[] headerChecksum,IPv4NetworkAddress sourceIp, IPv4NetworkAddress destIp  ) throws Exception {
		super();
